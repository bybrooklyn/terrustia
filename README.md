...
|---|:---:|:---:|:---:|
| Startup | 2.93 s | **0.45 s** | 7x faster |
| CPU, idle | 8.42% of a core | **2.30%** | 4x less |
| RAM, idle | 849.8 MB | **284.8 MB** | 3x less |
| Bandwidth over 5 minutes | 17,360 B | **0 B** | 100% less |

Measured 2026-09-08 by `tools/compare_vanilla.sh`, on one 8400x2400 world generated by the real
server and loaded by both, idle for five minutes each, on a machine quiet enough that the script's
own contention gate let the numbers stand. The table it prints is the table above; nothing here is
remembered or rounded by hand. The previous figures on this line were, and were wrong: they were
taken once with a method that read the macOS launcher's pid rather than the server's, which is why
they had vanilla at 104% of a core and terrustia at 45 MB.
