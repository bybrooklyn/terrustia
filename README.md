...
| 🟡 | Secret seeds (Celebrationmk10, Drunk World, Not the Bees, Remix, No Traps, "get fixed boi", Don't Starve, For the Worthy, Skyblock) | All nine detected by their real magic strings (an earlier pass had six of seven wrong: Remix's real trigger is `dontdigup`, Drunk World has only the numeric 5162020, and so on), fixed against source, plus two more the original investigation never named. **Eight of the nine now have real generation content**, landed 2026-09-08: No Traps (zero traps), Not the Bees (the world converted to hive and mud, water to honey), Don't Starve (wavy caves, marble piles, an opened lava layer), For the Worthy (the dungeon and temple painted, the sky islands corrupted), Celebrationmk10 (every landmark painted its own fixed colour), Drunk World (both evils, one per half), Skyblock (generation cancelled, islands kept), and "get fixed boi", which is a composite of the others. **Remix is the ninth and stays unclaimed.** It inverts depth across 211 call sites, and a flag telling a client the world is upside down when its tiles are not is a false claim about which way up it is (`secret_seed.rs:191-211`); a `.wld` real Terraria generated keeps its flag. An attempt to close it by mirroring the world left-to-right was written and deleted, because that is a different transformation that only looks like the seed. The real mechanism has started instead: `Layout::deep_band` answers "where is the cavern layer" and gives Remix vanilla's own inverted answer, with the gem-cave and spider-cave passes asking it. That is two passes of many, so the flag stays dropped until the surface, the underworld and the spawn move too. Each implemented seed is transcribed with the *other* seeds' branches taken as false, which is the plain single-seed path and is disclosed in every module |
