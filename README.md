...
| ✅ | Secret seeds (Celebrationmk10, Drunk World, Not the Bees, Remix, No Traps, "get fixed boi", Don't Starve, For the Worthy, Skyblock) | **All nine have real generation content**, landed 2026-09-08. No Traps (zero traps), Not the Bees (the world converted to hive and mud, water to honey), Don't Starve (wavy caves, marble piles, an opened lava layer), For the Worthy (the dungeon and temple painted, the sky islands corrupted), Celebrationmk10 (every landmark painted its own fixed colour), Drunk World (both evils, one per half), Skyblock (generation cancelled, islands kept), Remix (the rock layer driven to 0.6 of the world's height, the lava line moved the other way to a fifth below the surface, the player spawning on the underworld floor, the cavern content lifted above the rock line, the cave-wall depth mapping inverted and the dungeon brought inward), and "get fixed boi", the composite of the rest. Remix and zenith are now **claimed** on worlds this generator makes, which they were not before: the flag is a promise about a world's shape, and the tiles finally have that shape (`secret_seed.rs`). A test audits the claim against the evidence rather than trusting it. Each seed is transcribed with the *other* seeds' branches taken as false, which is the plain single-seed path; seed combinations are disclosed in every module as not modelled |
