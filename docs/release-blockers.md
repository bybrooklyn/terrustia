...
| Zero unknown protocol IDs | MET, evidence corrected 2026-09-08 | `docs/packet-ids.tsv` has no row whose status is `unknown`, which is what the clause asks. The evidence line here used to add "and no row that is `none`/`none`", and that was false: **19 rows are unimplemented on both sides**, each with a documented status - `dead-slot`, `deprecated`, `modding`, `social-host-only`, `not-applicable-dedicated`, `client-bookkeeping`. Unimplemented-with-a-reason is not the same as unknown, and the clause was always about the latter. Three of the nineteen are `live`: 100 TeleportNPCThroughPortal, 104 ShopOverride, 135 DeadPlayer. `tools/packet_audit.py` names them on every run and nothing gates on them; they are a real gap and are recorded here rather than left inside a sentence that said there were none |
...
| Human fresh-world Moon Lord playthrough | **WAIVED 2026-09-08, on the condition `TODO.md` itself sets** | `TODO.md` makes this "waivable only if the automated and differential evidence is otherwise complete", and when this file was written it was not: `check-data` was partially run and peak RSS was unmet. Both are met now, as is the README comparison row, so every other clause on this bar is met or explicitly carried to the next release. The automated evidence was also strengthened rather than merely re-run: `playbot` reports 9 of 9 goals in a fresh world and 4 of 4 after a save and reload, and one of those four is new. It used to declare "unlock a dungeon door with a Golden Key" permanently unreachable because "worldgen builds no dungeon", which was false - and driving it found two production bugs: the dungeon placed no door at all, and `smooth::kill_tile` was deleting frame-important tiles, which corrupted any multi-tile object it touched. The `playthrough` example was re-run 2026-09-09 against a fresh world and every link in the loot spine holds, Eye of Cthulhu through Moon Lord: Eye [56, 880], Eater of Worlds [86], Wall of Flesh [367], Destroyer [548], Skeletron Prime [547], Plantera [1141], Golem [1294], Lunatic Cultist [3549, 3372], Moon Lord reached. Two entries (Retinazer [549], Moon Lord [3460]) are in the tables and did not fall in four fights each, which is a chance roll rather than a missing drop. **A human playthrough is still strongly expected and nothing here substitutes for one**, and `docs/human-playthrough.md` is the checklist for doing it: what to look at, in the order a player meets it, and what to do with anything found. What this row records is that the waiver's own condition is now satisfied, not that the playthrough happened |
| README comparison table against the real server | **MET 2026-09-08** | Measured on a quiet machine (load 1.79, against the script's own 2.0 gate) and the README table replaced with what the tool printed: startup 2.93 s vs 0.45 s, idle CPU 8.42% vs 2.30%, idle RAM 849.8 MB vs 284.8 MB, bandwidth 17,360 B vs 0 B over five minutes. The old figures were wrong and are gone: they were hand-taken once with the method that read the macOS launcher's pid, which is why they claimed vanilla used 104% of a core. Getting here found **two** measurement defects in the tool itself. `bytes_out()` printed `0` both for a process that sent nothing and for a `nettop` that returned no rows, so an unmeasured run would have published as "100% less"; it says `UNMEASURED` now. And the startup timer was clocking terrustia against a disk still busy with vanilla's exit save, because the script killed vanilla's real process but only waited on its wrapper - that published 3.46 s where the same binary loads the same class of world in 0.46 s. Both were caught before publication, the second by the number simply looking backwards |
| Admin panel verified in a real browser | **MET 2026-09-08, and the clause was missing from this table until today** | `TODO.md`'s own final-verification list requires "the admin overhaul verified against a real client and Playwright" before tagging, and `AGENTS.md` asserted it as done - while the repository held no Playwright dependency, config, spec or test script, and this file, whose whole job is to record what was actually checked for every clause, did not carry the clause at all. Three documents, one claim, no implementation. `just check-panel` now boots the shipped release binary with the panel embedded and drives it in Chromium: four tests, all passing. The panel is served from its embedded copy and the Svelte app mounts; `/api/status` refuses an unauthenticated read with 401; a fresh server reports itself unclaimed; and a path outside the embedded tree never returns a file from disk. Two of those started as failures against *correct* server behaviour - the status route is authenticated, and an unknown path legitimately answers with the SPA shell - so the assertions were rewritten to test what actually matters after probing the server by hand. Deliberately outside `just check`, which should not assume a release build and a downloaded browser |
