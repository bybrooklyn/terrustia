...
| README comparison table against the real server | **NOT RUN: the tool works, the box does not** | `tools/compare_vanilla.sh` had a real measurement bug fixed 2026-09-04 (it read the macOS launcher's pid, not the server's). Run twice 2026-09-08 against the real 1.4.5.8 server: it completes end to end, produces the table, and **refuses its own output** (`NOT PUBLISHABLE: the machine was contended`) at load 7.58 and 3.91 against its 2.0 threshold. So this is no longer "nobody knows whether it runs" - it runs, and the remaining requirement is a quiet machine, which is not something a check can supply itself. The run also found a latent defect in it: `bytes_out()` printed `0` both for a process that genuinely sent nothing and for a `nettop` that returned no rows at all, and the README's "100% less" bandwidth row is computed from it, so an unmeasured run would have published as a perfect score. Fixed to say `UNMEASURED` instead. Verified it was not biting: `nettop` does return a row for an idle terrustia (`terrustia.<pid>,,,0,0`), and vanilla's own figure scales with the window (1960 B over 30 s, 17304 B over 300 s) while terrustia's stays 0, so the zero is measured |
